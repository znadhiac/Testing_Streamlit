...
Use the sidebar to enter customer details, then click **Predict** to evaluate their churn risk.  
This insight can help support proactive retention strategies and customer engagement efforts.
""")
...
st.markdown(
    ...
    <div style='text-align: center;'>
        <img src='ecommercepict.png' width='400'/>
    </div>
    """,
...
# --- LOAD MODEL ---
@st.cache_resource
def load_model(pickle_path='final_model_pipeline.pkl'):
    try:
...
if model is not None:
    ...
    if st.button('Predict'):
        ...
        if prediction == 1:
            st.error(f"Churn Prediction: **Yes** (Probability: {proba:.2f})")
            st.markdown("**Action Needed:** This customer is at risk of leaving. Consider targeted engagement.")
        else:
            st.success(f"Churn Prediction: **No** (Probability of Staying: {1 - proba:.2f})")
...
Purwadhika JCDSOL-018, Group Alpha<br>
</div>
""", unsafe_allow_html=True)
